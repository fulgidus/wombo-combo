 ...
 *
 * Defines the tasks parent command with all 9 subcommands using citty's
 * native subCommands support. Each subcommand delegates to the existing
 * implementation in `src/commands/tasks/`.
 *
 * Subcommands:
 *   list (ls)          — List tasks with optional filtering
 *   add (a)            — Add a new task
 *   set-status (ss)    — Change a task's status
 *   set-priority (sp)  — Change a task's priority
 *   set-difficulty (sd) — Change a task's difficulty
 *   check (ch)         — Validate tasks file
 *   archive (ar)       — Archive done/cancelled tasks
 *   show (sh)          — Show task details
 *   graph (g)          — Dependency graph visualization
 */
...
import { resolveOutputFormat, outputError } from "../../lib/output.js";
...
// ---------------------------------------------------------------------------
// Shared: load config, validate, ensure tasks file
// ---------------------------------------------------------------------------

async function loadProjectConfig(projectRoot: string) {
...
const listCommand = defineCommand({
  ...
  args: {
    status: {
      type: "string",
      description: "Filter by status (backlog|planned|in_progress|blocked|in_review|done|cancelled)",
      required: false,
    },
    priority: {
      type: "string",
      description: "Filter by priority (critical|high|medium|low|wishlist)",
      required: false,
    },
    difficulty: {
      type: "string",
      description: "Filter by difficulty (trivial|easy|medium|hard|very_hard)",
      required: false,
    },
    ready: {
      type: "boolean",
...
const checkCommand = defineCommand({
  meta: {
    name: "check",
    description: "Validate tasks file (schema, deps, duplicates, cycles) (also: ch)",
  },
  args: {
    output: {
      type: "string",
...
const archiveCommand = defineCommand({
  meta: {
    name: "archive",
    description: "Move done/cancelled tasks to archive (also: ar)",
  },
  args: {
    taskId: {
      type: "positional",
...
const graphCommand = defineCommand({
  meta: {
    name: "graph",
    description: "Visualize the task dependency graph (also: g)",
  },
  args: {
    status: {
      type: "string",
...
/**
 * Tasks parent command with all subcommands.
...
export const tasksCommand = defineCommand({
  ...
  subCommands: {
    ...
    show: showCommand,
    graph: graphCommand,
    // Aliases (same command objects, different keys)
    ls: listCommand,
    a: addCommand,
    ss: setStatusCommand,
    sp: setPriorityCommand,
    sd: setDifficultyCommand,
    ch: checkCommand,
    validate: checkCommand,
    ar: archiveCommand,
    sh: showCommand,
    g: graphCommand,
  },
